...
arrWrong["dvCode"] = "<img style='margin:2px;' src='"+imgpath+"images/zhuce2.gif'/>&nbsp;验证码位未输入或数输入不正确。";
arrWrong["dvPcode"] = "<img style='margin:2px;' src='"+imgpath+"images/zhuce2.gif'/>&nbsp;手机验证码未输入或不正确。";
...
$(document).ready(
function() {
	$('#dvRec').html('<font style="color:#777">填写推荐人用户名，可不填。</font>');
    Init();
    $("#txt_phone").focus();
...
function BlurIdCard(){
    ...
    if (idcartValidResult.indexOf('通过') == -1) {
        $(td).html(GetP("reg_wrong", "<img style='margin:2px;' src='"+imgpath+"images/zhuce2.gif'/>&nbsp;"+idcartValidResult));
    }else{
...
function BlurPwd() {
    var txt = "#txtPwd";
    var td = "#dvPwd";
    var pat = new RegExp("^(?!_)(?!.*?_$)[a-zA-Z0-9_]{6,16}$", "i");
    var str = $(txt).val();
    if (pat.test(str)) {
        //格式正确
...
function AsyncPVerCode(data) {
    if (data == "1") {
        ...
        $("#dvPcode").html(GetP("reg_wrong", "<img style='margin:2px;' src='"+imgpath+"images/zhuce2.gif'/>&nbsp;验证码未输入或填写错误！"));
...
function AsyncVerCode(data) {
    if (data == "1") {
        ...
        $("#dvCode").html(GetP("reg_wrong", "<img style='margin:2px;' src='"+imgpath+"images/zhuce2.gif'/>&nbsp;验证码未输入或填写错误！"));
		//$("#dvCode").html(GetP("reg_wrong", arrBox["dvCode"]));
    }
}
