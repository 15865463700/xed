...
function tborrow_status($borrow_id)
{
    $tvo = M("transfer_borrow_info")->where("id=".$borrow_id)->find();
    if($tvo['is_show'] == 0 && ($tvo['borrow_status'] == 7 || $tvo['borrow_status'] == 3))
    {
        $href =  '<a class="tz_btt">已完成</a>';
    }elseif($tvo['is_show'] == 1 && $tvo['transfer_out']!= $tvo['transfer_total']){
        $href =  '<a href="'.U('M/tinvest/detail', array('id'=>$borrow_id)).'"  class="tz_bt">马上投资</a> ';
    }else{
        $href =  '<a  class="tz_btt">还款中</a> ';
    }
...
function checkInvest($invest_uid, $borrow_id, $invest_money, $paypass, $invest_pass='')
{
    ...
    $borrow = M('borrow_info')
                ->field('id, borrow_uid,is_new, borrow_money, has_borrow, has_vouch, borrow_max,borrow_min,
                            borrow_type, password, money_collect, is_new')
                ->where("id='{$borrow_id}'")
                ->find();
    if($vm['money_collect'] > 0 && $borrow['is_new'] == 1 ){//新手标判断
        return L('not_invest_is_new');
    }
    if(!$borrow){ // 没有读取到借款数据
    ...
    $benefit = get_personal_benefit($invest_uid);
    if($borrow['is_new']==1&&$benefit["capital_collection"]>0){
        return L('not_invest_is_new');
    }
...
//            $money = $money-$vo['back_money'];
        ...
        if(($vo['all_money']-$money - $fee)<0 ){
            ...
            if($newid){
                memberMoneyLog($uid,4,-$money,"提现,默认自动扣减手续费".$fee."元",'0','@网站管理员@',0,$newid);
                MTip('chk6',$uid);
                return 'TRUE';
            } 
