...
class WithdrawlogwaitAction extends ACommonAction
{
    ...
    */
    ...
    {$map=array();
		...
		$list = M('member_withdraw w')->field($field)->join("shang_members m ON w.uid=m.id")->join("shang_member_info mi ON w.uid=mi.uid")->join("shang_member_money mm on w.uid = mm.uid")->where($map)->order(' w.id DESC ')->limit($Lsql)->select();
    ...
    public function edit() {
		...
		$list = M('member_withdraw w')->field($field)->join("shang_members m ON w.uid=m.id")->join("shang_member_info mi ON w.uid=mi.uid")->join("shang_member_money mm on w.uid = mm.uid")->join("shang_member_banks mb on w.uid = mb.uid")->where("w.id=$id")->order(' w.id ASC ')->limit($Lsql)->select();
		foreach($list as $v){
	 ...
	 public function doEdit() {
		...
		$vo = M("member_withdraw w")->field($field)->join("shang_member_money mm on w.uid = mm.uid")->find($id);
		...
		if($vo['withdraw_status']<>3 && $status==3){
			...
			memberMoneyLog($vo['uid'],12,$vo['withdraw_money'],"提现未通过,返还",'0','@网站管理员@',0,$id);
