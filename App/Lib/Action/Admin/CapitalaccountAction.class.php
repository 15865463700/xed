    ...
    */
	...
	public function export(){
		...
		foreach($list as $v){
				...
				$row[$i]['card_pass'] = $v['real_name']==""?"--":$v['real_name'];
				$row[$i]['card_mianfei'] = $v['money_freeze'] + $v['total_money'] + $v['money_collect'];
				$row[$i]['card_mianfei1'] = $v['total_money']==""?"0":$v['total_money'];
				$row[$i]['card_mianfei2'] = $v['money_freeze']==""?"0":$v['money_freeze'];
				$row[$i]['dsbx'] = $v['xmoney']['dsbx']==""?"0":$v['xmoney']['dsbx'];
				$row[$i]['dsbj'] = $v['xmoney']['dsbj']==""?"0":$v['xmoney']['dsbj'];
				$row[$i]['dslx'] = $v['xmoney']['dslx']==""?"0":$v['xmoney']['dslx'];
				
				$row[$i]['dfbx'] = $v['xmoney']['dfbx'];
				$row[$i]['dfbj'] = $v['xmoney']['dfbj']==""?"0":$v['xmoney']['dfbj'];
				$row[$i]['dflx'] = $v['xmoney']['dflx']==""?"0":$v['xmoney']['dflx'];
				$row[$i]['dxrtb'] = $v['xmoney']['dxrtb']==""?"0":$v['xmoney']['dxrtb'];
				$row[$i]['dshtx'] = $v['xmoney']['dshtx'];
				$row[$i]['clztx'] = $v['xmoney']['clztx'];
				
				$row[$i]['ljtxsxf'] = $v['xmoney']['ljtxsxf']==""?"0":$v['xmoney']['ljtxsxf'];
				$row[$i]['ljczsxf'] = $v['xmoney']['ljczsxf']==""?"0":$v['xmoney']['ljczsxf'];
				$row[$i]['ljtxje'] = $v['xmoney']['ljtxje']==""?"0":$v['xmoney']['ljtxje'];
				$row[$i]['ljczje'] = $v['xmoney']['ljczje']==""?"0":$v['xmoney']['ljczje'];
				$row[$i]['ljzfyj'] = $v['xmoney']['ljzfyj']==""?"0":$v['xmoney']['ljzfyj'];
				$row[$i]['ljtbjl'] = $v['xmoney']['ljtbjl']==""?"0":$v['xmoney']['ljtbjl'];
				$row[$i]['jzlx'] = $v['xmoney']['jzlx']==""?"0":$v['xmoney']['jzlx'];
				$row[$i]['jflx'] = $v['xmoney']['jflx']==""?"0":$v['xmoney']['jflx'];
				$row[$i]['glycz'] = $v['xmoney']['glycz']==""?"0":$v['xmoney']['glycz'];
				$i++;
			}
