...
class BorrowAction extends ACommonAction
{
    ...
    */
    ...
    public function waitverify2()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.has_borrow,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.full_time,m.user_name,m.id mid,b.is_tuijian,b.money_collect';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function repaymenting()
    {
		...
		$field= 'm.id as mid,m.customer_name,b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.has_borrow,b.borrow_interest,b.repayment_money,b.repayment_interest,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.deadline,m.user_name,m.user_phone,b.is_tuijian,b.money_collect';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function done()
    {
		...
		$field= 'b.id,b.borrow_name,b.has_borrow,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.repayment_money,b.deadline,m.id mid,m.user_name';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
	...
	public function doEditWaitverify(){
		...
		$vm = M('borrow_info')->field('borrow_uid,is_new,borrow_status,borrow_type,first_verify_time,password,updata,danbao,vouch_money,money_collect,can_auto')->find($m->id);

        $m->is_new = $_POST["is_new"];
	...
	public function doEditWaitverify2(){
		...
		$vm = M('borrow_info')->field('borrow_uid,is_new,has_borrow,borrow_money,has_borrow,borrow_status,first_verify_time,updata,danbao,vouch_money,borrow_fee,borrow_interest_rate,borrow_duration,repayment_type,collect_day,collect_time,money_collect')->find($m->id);
		...
		if($vm['borrow_money']<>$m->borrow_money ||
            ...
            ((!isset($_POST["is_new"])&&1 ==$vm['is_new'] )||($_POST["is_new"]==1&&1 <>$vm['is_new'] ))
		  ){
			$this->error('复审中的借款不能再更改‘还款方式’，‘借款金额’，‘年化利率’，‘借款期限’,‘借款管理费’,‘是否是新手标’');
			exit;
		}
		...
		if(($vm['borrow_money']!=$capital_sum2)){
            ...
            if($vm['repayment_type'] == 1){//按天还
                $fee_rate = (is_numeric($fee_borrow_manage[0]))?($fee_borrow_manage[0]/100):0.001;
                $borrow_fee_ = getFloatValue($fee_rate*$vm['borrow_money']*$vm['borrow_duration'],2);
            }else{
                ...
                if($vm['borrow_duration']>$fee_borrow_manage[3]&&is_numeric($fee_borrow_manage[3])){
                    $borrow_fee_ = getFloatValue($fee_rate_1*$vm['borrow_money'],2);
                    $borrow_fee_ += getFloatValue($fee_rate_2*$vm['borrow_money']*($vm['borrow_duration']-$fee_borrow_manage[3]),2);
                }else{
                    $borrow_fee_ = getFloatValue($fee_rate_1*$vm['borrow_money'],2);
                }
            }
            ...
            if($vm['repayment_type'] == 1){//按天还
                $fee_rate = (is_numeric($fee_borrow_manage[0]))?($fee_borrow_manage[0]/100):0.001;
                $borrow_fee = getFloatValue($fee_rate*$vm['has_borrow']*$vm['borrow_duration'],2);
            }else{
                $fee_rate_1=(is_numeric($fee_borrow_manage[1]))?($fee_borrow_manage[1]/100):0.02;
                $fee_rate_2=(is_numeric($fee_borrow_manage[2]))?($fee_borrow_manage[2]/100):0.002;
                if($vm['borrow_duration']>$fee_borrow_manage[3]&&is_numeric($fee_borrow_manage[3])){
                    $borrow_fee = getFloatValue($fee_rate_1*$vm['has_borrow'],2);
                    $borrow_fee += getFloatValue($fee_rate_2*$vm['has_borrow']*($vm['borrow_duration']-$fee_borrow_manage[3]),2);
                }else{
                    $borrow_fee = getFloatValue($fee_rate_1*$vm['has_borrow'],2);
                }
            }
            ...
            /**未满标重新计算借款管理费*/
            if($borrow_fee_==$vm['borrow_fee']){
                $m->borrow_fee = $borrow_fee;
            }

            #计算服务费
            #borrow_service_fee-$fee_borrow_service
            $vm['borrow_service_fee'] = $vm['has_borrow'] * $fee_borrow_service[0] * 0.01;
            if($vm['borrow_service_fee']>$fee_borrow_service[1]){
                $m->borrow_service_fee = $fee_borrow_service[1];
            }
	...
	public function doEditWaitmoney(){
		...
		$vm = M('borrow_info')->field('borrow_uid,is_new,borrow_type,has_borrow,borrow_money,first_verify_time,borrow_interest_rate,borrow_duration,repayment_type,collect_day,collect_time,borrow_fee,money_collect')->find($m->id);
        ...
        if($vm['borrow_money']<>$m->borrow_money ||
			...
			$this->error('招标中的借款不能再更改‘还款方式’，‘借款种类’，‘借款金额’，‘年化利率’，‘借款期限’,‘借款管理费’,‘是否是新手标’');
			exit;
		}
		...
		if($m->borrow_status==3){
			...
			}else if($vm['collect_day'] > $m->collect_day){
                $temp_time = strtotime("+ {$m->collect_day} day",$vm['first_verify_time']);
                if($temp_time<time()){
                    $this->error('募集期不能小于已招标时间！');
                }
                $m->collect_time = $temp_time;
            }
    ...
    public function export(){
        ...
        $map['borrow_id'] = $_REQUEST['borrow_id'];
        $Bconfig = require C("APP_ROOT")."Conf/borrow_config.php";
