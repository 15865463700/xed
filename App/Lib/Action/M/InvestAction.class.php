    ...
    class InvestAction extends HCommonAction
    {
        ...
        {   
            ...
            $borrowinfo = M("borrow_info bi")
                            ...
                            ->join('shang_article ac on ac.id= bi.danbao')
                            ->where('bi.id='.$id)
                            ->find();
...
//            exit;
            ...
            if($this->isAjax()){   // ajax提交投资信息
                ...
                $borrow_info = M("borrow_info")
                    ->field('borrow_duration, borrow_money, borrow_interest, borrow_interest_rate, has_borrow,
                             borrow_min, borrow_max, password, repayment_type, is_new,password')
                    ->where("id='{$borrow_id}'")
                    ->find();
                ...
                $user_info = M('member_money')
                                ->field("account_money+back_money as money, money_collect")
                                ->where("uid='{$this->uid}'")
                                ->find();
                if(!is_array($user_info)){
                    $user_info['money_collect'] = 0;
                }
        ...
        */
        public function get_coupon_list(){
            ...
            $list = get_unused_interestratecoupon_list($this->uid, $borrow_id, $money);
