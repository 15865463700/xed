...
//        $this->display();
    ...
    public function idcard() {
        ...
        $shangid5config = FS("Dynamic/shangid5");
        $this->assign("id_status", $ids);
        $this->assign("id5_status", $id5config['enable']);
        $this->assign("shangid5_status",$shangid5config['enable']);
        $data['html'] = $this->fetch();
        exit(json_encode($data));
    }
...
//        if($c > 0) ajaxmsg("该身份证号已被使用",0);
        ...
        $id5config = FS("Dynamic/id5");
        $shangid5config = FS("Dynamic/shangid5");
        if ($id5config['enable'] == 0 && $shangid5config['enable'] == 0) {
//            if ($isimg != 1)
...
//                ajaxmsg("请先上传身份证反面图片", 0);
        ...
        }elseif($shangid5config['enable'] == 1){
            $userid = $shangid5config['USERID'];//userid
            $pass = $shangid5config['KEY'];//password
            $realname = $_REQUEST['realname'];
            $idcard = $_REQUEST['idcard'];
            $shangid5_result = file_get_contents("http://realname.tuanshang.net/Api/api.php?userid={$userid}&password={$pass}&realname={$realname}&idcard={$idcard}",false);
            $result = json_decode($shangid5_result,true);
            if($result['code']==200){
                $c = M('member_info')->where("uid = {$this->uid}")->count('uid');
                if ($c == 1) {
                    $newid = M('member_info')->where("uid = {$this->uid}")->save($data);
