...
//		$this->create($submitdata,"http://ceshi.allinpay.com/gateway/index.do" );//测试环境
	...
	public function allinpaywap()
	{
		...
		unset( $this->paydetail['bank'] );
		$this->paydetail['fee'] = getfloatvalue( $allinppaywap_config['feerate'] * $this->paydetail['money'] / 100, 2 );
		$this->paydetail['nid'] = $this->createnid( "allinpaywap", $submitdata['orderNo'] );
...
//		$this->create($submitdata,"http://ceshi.allinpay.com/mobilepayment/mobile/SaveMchtOrderServlet.action" );//测试环境
		$this->create($submitdata,"http://ceshi.allinpay.com/mobilepayment/mobile/SaveMchtOrderServlet.action" );//生产环境
	}
	...
	public function paynotice(){
		...
		switch($payid){
			...
			break;
				...
				if($payResult == "1"){
					$signGet = $this->getSign( "allinpay_return", $_REQUEST );
                                        file_put_contents("signGet.txt",$signGet);
					$nid = $this->createnid( "allinpaywap", $_REQUEST['orderNo'] );
					if ($signGet){
						$done = $this->payDone( 1, $nid, $_REQUEST['orderNo'] );
					}else{
						$done = $this->payDone( 2, $nid, $_REQUEST['orderNo'] );
	...
	private function payDone($status,$nid,$oid){
		...
		switch($status){
			case 1:
				...
				$vo = M('member_payonline')->field('uid,money,fee,status,add_time')->where("nid='{$nid}'")->find();
				...
				SMStip("payonline",$vx['user_phone'],array("#USERANEM#","#DATE#","#MONEY#"),array($vx['user_name'],date('Y-m-d H:i:s',$vo['add_time']),$vo['money']));
			break;
			case 2:
				$updata['status'] = $status;
				$updata['tran_id'] = text($oid);
