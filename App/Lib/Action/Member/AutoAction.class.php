...
class AutoAction extends MCommonAction {
    ...
    public function auto(){
		...
		if($vo){
			$vocount = M()->query("SELECT count( `id` ) num FROM `shang_auto_borrow` WHERE `borrow_type` =1
				AND `invest_time` < ( 
				SELECT `invest_time` 
				FROM `shang_auto_borrow` 
		...
		if($vot){
			$votcount = M()->query("SELECT count( `id` ) tnum FROM `shang_auto_borrow` WHERE `borrow_type` =3
				AND `invest_time` < ( 
				SELECT `invest_time` 
				FROM `shang_auto_borrow` 
