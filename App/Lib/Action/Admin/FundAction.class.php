...
//		$this->assign("borrow_type", $btype );
		...
		$this->assign("borrow_name","U-".str_repeat("0",8-strlen($Newid)).$Newid);
	}
...
//        echo "model2==".$result2;exit;
		if ($result && $result2) { //保存成功
		  ...
		  //自动投标
            autotInvest($result);
			alogs("Tborrow",$result,1,'成功执行了优计划的添加操作！');//管理员操作日志
		  //成功提示
			$this->assign('jumpUrl', __URL__);
			$this->success(L('新增成功'));
		}else{
			alogs("Tborrow",$result,0,'执行优计划的添加操作失败！');//管理员操作日志
			$model->rollback();
			//失败提示
			$this->error(L('新增失败'));
		}
...
//		}
    ...
    public function export(){
        ...
        $field= 'bi.id bid,b.id,bi.investor_capital,bi.investor_interest,bi.invest_fee,bi.add_time,m.user_name,m.id mid,m.user_phone,b.borrow_duration,b.repayment_type,m.customer_name,b.borrow_name,bi.transfer_month,b.borrow_type';
        $list = M('transfer_borrow_investor bi')->field($field)->join("{$this->pre}members m ON m.id=bi.investor_uid")->join("{$this->pre}transfer_borrow_info b ON b.id=bi.borrow_id")->where($map)->limit($Lsql)->order("bi.id DESC")->select();
        $row=array();
