...
class CapitalonlineAction extends ACommonAction
{
    ...
    */
    public function charge()
    {
        ...
        $this->assign("way", array('off'=>'线下充值','gfb'=>'国付宝','ips'=>'环迅支付','chinabank'=>'网银在线','baofoo'=>'宝付','tenpay'=>'财付通','ecpss'=>'汇潮支付','easypay'=>'易生支付','cmpay'=>'中国移动支付','allinpay'=>'通联支付','baofuwap'=>'宝付认证支付(wap)'));
...
//		if(session('admin_is_kf')==1)	$map['m.customer_id'] = session('admin_id');
     ...
     public function withdraw()
    {
		...
		$list = M('member_withdraw w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->join("shang_member_money mm on w.uid = mm.uid")->where($map)->limit($Lsql)->order("w.id DESC")->select();
